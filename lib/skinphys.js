...
(function (root, factory) {
  ...
  // Shared impact clock: strength, recovery tau and the full splat
  // duration (attack, hold, then decay to under 1%), so the window
  // dwell and the crush envelope can never disagree.
  function impactTiming(speed) {
    const k = Math.min(1, Math.max(0.2, ((speed || 900) - 300) / 2300));
    const tau = 40 + 40 * k;
    return { k, tau, total: 90 + 5 * tau };
  }

  return { createSkin, impactTiming };
});
