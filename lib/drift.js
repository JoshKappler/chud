...
function dragSpeed() {
  ...
  const sx = recent.reduce((a, s) => a + s.dx, 0);
  const sy = recent.reduce((a, s) => a + s.dy, 0);
  return Math.hypot(sx, sy) / span;
}
...
// Drag moves route through here so held bashes against a real monitor
// edge clamp at the wall and hurt; crossing into another display stays
// free. Same hurt path and cooldown as fling bounces.
function dragMove(dx, dy) {
  ...
  let hit = null;
  if (nx < wa.x && !displayBeyond(wa.x - 20, ny + h / 2, disp.id)) {
    nx = wa.x;
    hit = { edge: 'left', ix: wa.x, iy: ny + h / 2 };
  }
  if (nx + w > wa.x + wa.width && !displayBeyond(wa.x + wa.width + 20, ny + h / 2, disp.id)) {
    nx = wa.x + wa.width - w;
    hit = { edge: 'right', ix: wa.x + wa.width, iy: ny + h / 2 };
  }
  if (ny < wa.y && !displayBeyond(nx + w / 2, wa.y - 20, disp.id)) {
    ny = wa.y;
    hit = { edge: 'top', ix: nx + w / 2, iy: wa.y };
  }
  if (ny + h > wa.y + wa.height && !displayBeyond(nx + w / 2, wa.y + wa.height + 20, disp.id)) {
    ny = wa.y + wa.height - h;
    hit = { edge: 'bottom', ix: nx + w / 2, iy: wa.y + wa.height };
  }
  win.setPosition(Math.round(nx), Math.round(ny));
  if (hit) {
    const sp = dragSpeed();
    if (sp > 800 && Date.now() - lastHurtAt > 500) {
      lastHurtAt = Date.now();
      if (onBounce) onBounce({ ...hit, speed: sp, wa });
    }
  }
}
