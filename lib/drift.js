...
function tick() {
  ...
  if (mode === 'fling') {
    ...
    px += vx * dt;
    py += vy * dt;
    if (px < wa.x) { px = wa.x; vx = Math.abs(vx) * BOUNCE; }
    if (px + w > wa.x + wa.width) { px = wa.x + wa.width - w; vx = -Math.abs(vx) * BOUNCE; }
    if (py < wa.y) { py = wa.y; vy = Math.abs(vy) * BOUNCE; }
    if (py + h > wa.y + wa.height) { py = wa.y + wa.height - h; vy = -Math.abs(vy) * BOUNCE; }
...
module.exports = { start, setDragging, noteMove, stop };
