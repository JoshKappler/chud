...
// Dragging chases the cursor one move per displayed frame: the renderer
// frame clock is display-link aligned, while a free-running timer beats
// against the refresh rate, which is the freeze-then-catch-up stutter.
// The fallback only covers a renderer that stops ticking mid-drag.
let dragGrab = null;
...
function beginDrag() {
  ...
  const [wx, wy] = win.getPosition();
  dragGrab = { offX: c.x - (wx + inset), offY: c.y - (wy + inset), ax: c.x, ay: c.y, armed: false };
  lastTickAt = Date.now();
  dragFallback = setInterval(() => {
    if (Date.now() - lastTickAt > 40) dragTick();
  }, 50);
}
...
function dragTick() {
  ...
  if (!dragGrab.armed) {
    if (Math.abs(p.x - dragGrab.ax) + Math.abs(p.y - dragGrab.ay) <= 3) return;
    dragGrab.armed = true;
  }
  const [cwx, cwy] = win.getPosition();
  const dx = p.x - dragGrab.offX - (cwx + inset);
  const dy = p.y - dragGrab.offY - (cwy + inset);
