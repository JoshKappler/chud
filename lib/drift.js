...
function tick() {
  ...
  if (mode === 'fling') {
    // A hurting hit pins him to the wall while the crush plays out; the
    // reflected velocity is banked and released when the dwell ends.
    if (dwellUntil) {
    ...
    if (hit && preSpeed > 900 && Date.now() - lastHurtAt > HURT_COOLDOWN_MS) {
      ...
      vx = 0;
      vy = 0;
      dwellUntil = Date.now() + 140 + 120 * Math.min(1, (preSpeed - 300) / 2300);
    }
