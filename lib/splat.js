...
function show(edge, ix, iy, wa, speed) {
  ...
  lastAt = now;
  init();
  // Route by the bounce's own workArea: the impact point sits exactly on
  // the boundary line, outside every display, and matching that 1px rect
  // picks an arbitrary neighbor (blood landed on the wrong monitor).
  const disp = wa ? screen.getDisplayMatching(wa)
    : screen.getDisplayMatching({ x: Math.round(ix), y: Math.round(iy), width: 1, height: 1 });
